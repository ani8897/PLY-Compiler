...
def p_expression(p):
	...
	if len(p) == 2:
		...
		if p[1].syminfo.type == None:
			...
			if status and not(p[1].syminfo.is_int or p[1].syminfo.is_float) and symindirection == 0 and p[1].syminfo.indirection == 0:
				raiseDirectUseOfNonPointer(p[1].syminfo.var_name,glob.line_number)
